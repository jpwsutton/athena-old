...
if (config.readOnlyMode) {
  console.log("In Read only mode. No Subscriptions will be made");
  ...
  db.collection('topics', function(err, collection) {
    collection.findOne({
      ...
      if (item === null) {
        ...
        collection.insert(topicRecord, {
          safe: true
        }, function(err, result) {
          if (err) {
            logError(err);
          } else {
            logMessage("Success! " + topicRecord.topic +
              " added to database");
          }
        });
      }
